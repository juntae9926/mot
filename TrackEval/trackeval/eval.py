        ...
        for dataset, dataset_name in zip(dataset_list, dataset_names):
            ...
            for tracker in tracker_list:
                ...
                try:
                    ...
                    logger.info('Tracker %s was unable to be evaluated.' % tracker)
                    logger.info(err)
                    traceback.print_exc()
                    if config['LOG_ON_ERROR'] is not None:
                        with open(config['LOG_ON_ERROR'], 'a') as f:
                            logger.info(dataset_name, file=f)
                            logger.info(tracker, file=f)
                            logger.info(traceback.format_exc(), file=f)
                            logger.info('\n\n\n', file=f)
