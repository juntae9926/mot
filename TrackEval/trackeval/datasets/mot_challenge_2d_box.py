...
class MotChallenge2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        self.gt_set = gt_set
        if 'seqmap_tag' in config.keys():
            self.gt_set_seqmap = self.config['BENCHMARK'] + '-' + self.config['seqmap_tag']
        else:
            self.gt_set_seqmap = gt_set
        ...
        if self.config["SEQ_INFO"]:
            ...
            if self.config["SEQMAP_FILE"]:
                ...
                if self.config["SEQMAP_FOLDER"] is None:
                    seqmap_file = os.path.join(self.config['GT_FOLDER'], 'seqmaps', self.gt_set_seqmap + '.txt')
                else:
                    seqmap_file = os.path.join(self.config["SEQMAP_FOLDER"], self.gt_set_seqmap + '.txt')
